...
export async function POST(req) {
  try {
    ...
    if (!DID_KEY) {
      return NextResponse.json({ error: "D-ID key not set" }, { status: 500 });
    }

    if (!IMAGE_URL) {
      return NextResponse.json(
        { error: "imageUrl or AVATAR_SOURCE_IMAGE_URL required" },
        { status: 400 }
      );
    }
    ...
    if (!didResp.ok) {
      return NextResponse.json(
        { error: data ?? `D-ID ${didResp.status}` },
        { status: 502 }
      );
    }

    // Return the job data (likely contains job id)
